...
::: {.callout-tip}
## 📊 Click the infographic to zoom & pan!
:::
...
│    └─────┘      (every 30s)            └─────┘                           │
│                                                                          │
│    Problem: Multicast often blocked on WiFi, NAT, cloud                 │
└─────────────────────────────────────────────────────────────────────────┘
...
│    └─────┘         └──────┘          └─────┘                            │
│                                                                          │
│    Solution: Router-based, works over any TCP connection                │
└─────────────────────────────────────────────────────────────────────────┘
```
:::

### 🎯 The Eureka Moment: "Why Isn't My Listener Receiving?"
...
```
┌─────────────────────────────────────────────────────────────┐
│                   ZENOH STARTUP SEQUENCE                     │
│                                                              │
│   Step 1: Start the Router (Terminal 1)                     │
│         ┌──────────────┐                                    │
│         │ rmw_zenohd   │  ← Must start FIRST               │
│         │ listening on │                                    │
│         │ tcp://:7447  │                                    │
│         └──────┬───────┘                                    │
│                │                                             │
│   Step 2: Nodes connect to router                           │
│       ┌────────┴────────┐                                   │
│       │                 │                                    │
│       ▼                 ▼                                    │
│   ┌───────┐        ┌────────┐                               │
...
│   └───────┘        └────────┘                               │
│                                                              │
│   Now they discover each other via the router!              │
└─────────────────────────────────────────────────────────────┘
```
:::
...
|----------|-----------------|----------------|
| Same LAN | ✅ Works | ✅ Works |
| WiFi (multicast often blocked) | ❌ Fails | ✅ Works |
| Across subnets | ❌ Fails | ✅ Works |
| Through NAT/firewall | ❌ Fails | ✅ Works |
| Cloud deployment | ❌ Fails | ✅ Works |
...
│                    CONNECT TO ROBOT                                      │
│                                                                          │
│    Your Workstation                          Robot (192.168.1.2)        │
│    ────────────────                          ──────────────────          │
│                                                                          │
│    ┌──────────────┐                          ┌──────────────┐           │
│    │  rmw_zenohd  │───── TCP connection ────►│  rmw_zenohd  │           │
│    │  (client)    │         :7447            │  (router)    │           │
│    └──────┬───────┘                          └──────┬───────┘           │
│           │                                         │                    │
│    ┌──────┴───────┐                          ┌──────┴───────┐           │
│    │   rviz2      │                          │  camera_node │           │
│    │   teleop     │◄─────── topics ─────────►│  lidar_node  │           │
│    │   rqt        │                          │  odom_node   │           │
│    └──────────────┘                          └──────────────┘           │
│                                                                          │
│    ZENOH_CONFIG_OVERRIDE='connect/endpoints=["tcp/192.168.1.2:7447"]'   │
└─────────────────────────────────────────────────────────────────────────┘
```
:::
...
::: {.callout-note collapse="true"}
## 🔍 Deep Dive: What is the Gossip Protocol?
...
│                                                                          │
│   Step 1: Router A connects to Router B                                 │
│   ┌─────┐         ┌─────┐                                               │
│   │  A  │────────►│  B  │     A tells B: "I know about router C"       │
│   └─────┘         └─────┘                                               │
│                                                                          │
│   Step 2: Router B now knows about C (even without direct connection)   │
│   ┌─────┐         ┌─────┐         ┌─────┐                               │
│   │  A  │────────►│  B  │ · · · · │  C  │   B can now connect to C!    │
│   └─────┘         └─────┘         └─────┘                               │
│                                                                          │
│   Step 3: Information spreads exponentially                             │
│   ┌─────┐         ┌─────┐         ┌─────┐                               │
│   │  A  │◄───────►│  B  │◄───────►│  C  │   Full mesh formed!          │
│   └──┬──┘         └─────┘         └──┬──┘                               │
│      │                               │                                   │
│      └───────────────────────────────┘                                  │
└─────────────────────────────────────────────────────────────────────────┘
...
|----------|--------------------------|----------------------|
| Reliable Pub → Reliable Sub | ✅ Works | ✅ Works |
| Reliable Pub → Best Effort Sub | ✅ Works | ✅ Works |
| **Best Effort Pub → Reliable Sub** | ❌ **Silent failure** | ✅ **Works!** |
| Best Effort Pub → Best Effort Sub | ✅ Works | ✅ Works |
...
│                                                                          │
│    Publisher           Network Stack          Subscriber                 │
│    ┌─────────┐        ┌───────────┐         ┌─────────┐                 │
│    │ 720p    │──copy──│  Kernel   │──copy───│ 720p    │                 │
│    │ Image   │        │  Buffer   │         │ Image   │                 │
│    │ (2.7MB) │        │           │         │ (2.7MB) │                 │
│    └─────────┘        └───────────┘         └─────────┘                 │
│                                                                          │
│    Total copies: 2+ (publisher→kernel→subscriber)                       │
│    CPU overhead: HIGH for large messages                                 │
└─────────────────────────────────────────────────────────────────────────┘
...
│    └─────────┘        └───────────┘         └─────────┘                 │
│                                                                          │
│    Total copies: 0 (pointer exchange only!)                             │
...
```bash
# 1280×720 (720p) - What happens at real camera resolution?
...
```bash
# 1920×1080 (1080p) - Full HD, the worst case
...
|------------|---------------|------------|-----------|--------|
| **320×240** | 30 | **29.6** | 7 MB/s | ✅ Full speed |
| **640×480** | 30 | **29.6** | 28 MB/s | ✅ Full speed |
| **1280×720** | 30 | **9.9** | 28 MB/s | ⚠️ **10 FPS only** |
| **1920×1080** | 30 | **5.0** | 32 MB/s | 🔴 **5 FPS only** |
...
$ ros2 topic hz /image
average rate: 9.883  # Still 10 FPS! 🤔
```
...
```
Synthetic (cam2image):   CPU → slow frame generation → transport doesn't matter
Real camera (D435i):     Hardware → fast 30 FPS → transport becomes bottleneck!
```

::: {.callout-tip}
## When SHM Actually Helps
- ✅ **Real cameras** (hardware generates frames fast)
- ✅ **Pre-recorded rosbags** (data already exists)
- ✅ **GPU-accelerated pipelines** (CUDA generates frames fast)
...
│    └─────────┘                       └───────────┘                       │
│                                                                          │
│    Bottleneck: CPU-bound pixel generation                               │
└─────────────────────────────────────────────────────────────────────────┘
...
│    Hardware                          Transport                           │
│    ┌─────────┐                       ┌───────────┐                       │
│    │ D435i   │  30 FPS               │ 83 MB/s   │                       │
│    │ sensor  │─────────fast!─────────│  flowing  │                       │
│    │ (0% CPU)│       ⚡               │           │                       │
│    └─────────┘                       └───────────┘                       │
│                                                                          │
│    Bottleneck: NONE - transport handles it easily!                      │
└─────────────────────────────────────────────────────────────────────────┘
```
:::
...
|----------|-------------|-----|
| **Multiple HD cameras** | ✅ Throughput | 3× D435i @ 1080p = 564 MB/s |
| **Latency-critical control** | ✅ Latency | ~0.5ms vs ~3ms |
| **CPU-constrained robots** | ✅ CPU savings | Eliminates copy overhead |
| **Point clouds** | ✅ Both | 10-50 MB messages @ 10+ Hz |
...
1. **CPU-bound generation?** → Faster CPU or GPU acceleration
2. **Transport-bound?** → SHM for local, compression for remote
3. **Subscriber-bound?** → Faster processing or lower rate
...
|------------|------------|----------|----------|------------|
| **320×240** | 0.23 MB | 7 MB/s @ 30 FPS | ✅ **7.02 MB/s @ 30 FPS** | None |
| **640×480** | 0.92 MB | 28 MB/s @ 30 FPS | ✅ **27.55 MB/s @ 30 FPS** | None |
| **1280×720** | 2.76 MB | 83 MB/s @ 30 FPS | ⚠️ **27.37 MB/s @ 10 FPS** | **CPU** |
| **1920×1080** | 6.22 MB | 186 MB/s @ 30 FPS | 🔴 **31.89 MB/s @ 5 FPS** | **CPU** |

**Real Camera (D435i) - No Bottleneck:**

| Resolution | Frame Size | Expected | Measured | Bottleneck |
|------------|------------|----------|----------|------------|
| **1280×720** | 2.76 MB | 83 MB/s @ 30 FPS | ✅ **83 MB/s @ 30 FPS** | None |
| **1920×1080** | 6.22 MB | 186 MB/s @ 30 FPS | ✅ **188 MB/s @ 30 FPS** | **None!** |
...
| **Multiple HD cameras** | ✅ Yes | 3× cameras = 500+ MB/s, reduces CPU |
| **Latency-critical control** | ✅ Yes | SHM gives ~0.5ms vs ~3ms latency |
| **Point clouds** | ✅ Yes | 10-50 MB messages benefit from zero-copy |
...
| `--shm-size=1g` | 1GB /dev/shm | ✅ SHM works |
| `--ipc=host` | Uses host's shared memory | ✅ SHM works (less isolated) |
